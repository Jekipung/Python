...
window.addEventListener('DOMContentLoaded', function() {
    ...
    loadCategories().then(function() {
        // category_id와 currPage가 URL로 전달된 경우
...
function setupPageList() {
    ...
    let pervBtn  = document.querySelector('#prev-page');
    ...
    if (prevNum <= 0) {
        pervBtn.classList.add('disabled');
    } else {
        pervBtn.classList.remove('disabled');
        pervBtn.onclick = function() { setPage(prevNum)}
    }
