 ...
 * LCD 제어 관련 코드들
 */
LiquidCrystal_I2C lcd(0x27, 16, 2); // LCD 제어객체 생성
...
void lcd_print(int humi, int temp, float dust) {
  ...
  // ----------- 미세먼지 출력 -------------
  // 실수를 문자열로 바꾸는 함수 (실수값, 정수부 길이, 소수부 길이, 문자열 버퍼)
...
void dust_setup() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    delayMicroseconds(DUST_STOP_TIME);    // 6. 다음 재측정까지 9680 마이크로초(9.68 밀리초) 동안 대기한다.
  }
...
float dust_read() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // 최근 저장된 마지막 슬롯을 제외한 나머지는 한칸씩 앞으로 땡긴다.
    // (즉, 가장 나중에 측정된 값은 버린다.)
 ...
 * LED 모듈 제어 관련 코드들
 */
#define LED_R_PIN 5   // LED 모듈 RED 핀번호
...
void serial_wait_start() {
  while(!isRun) {
    ...
    if (Serial.available()) {
      ...
      buffer[i - 1] = 0; // 마지막으로 들어온 줄바꿈 문자는 지운다.
...
![image](https://user-images.githubusercontent.com/94054859/233762456-a591b13a-ee5d-4144-88ab-44bca4fec62c.png)
